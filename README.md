...
응시자가 처음 보는 것은 시험 목록이 아니라 **사무실 한 층**입니다. 복도가 있고, 직군별로 방이 있고, 방 안에 자리가 있습니다. 지원한 팀으로 걸어 들어가 자리에 앉으면 그 자리의 일이 시작됩니다.
...
**방 목록은 이 저장소에 박혀 있지 않습니다.** 회사마다 뽑는 직군이 다르므로, 관리자가 콘솔에서 부서를 만들고 이름과 색과 순서를 정하면 평면도가 그 목록에서 생성됩니다. 기본으로 여덟 개(AI 플랫폼·데이터 엔지니어링·데이터 분석·프로덕트·고객경험·비즈니스 운영·피플·재무구매)가 들어 있지만, 그건 지우고 바꾸라고 넣어 둔 것입니다.
